...
//           interviewsRaw.map(async (interview: any) => {
//             try {
//               // Fetch full interview doc which includes child table data
//               const docRes = await fetch(
//                 `${API_BASE_URL}/api/resource/Interview/${interview.name}`,
//                 {
//                   method: 'GET',
//                   credentials: 'include',
...
//               const interviewers = (docData.data?.interview_details || [])
//                 .map((d: any) => d.interviewer)
//                 .filter(Boolean)
//               return { ...interview, interviewers }
//             } catch (error) {
//               return { ...interview, interviewers: [] }
...
//               <Link href="/home" className="ip-nav-link">
//                 <Home size={15} /> Home
//               </Link>
//               <div className="ip-nav-lbl">Pipeline</div>
//               {sidebarPipeline.map(s => (
...
//                                   <button className="ip-btn-sm-blue" onClick={e => {
//                                     e.stopPropagation()
//                                     // Pass cleared rounds so Event page can block them
//                                     const clearedRounds = allInterviews
//                                       .filter(i => i.job_applicant === candidate.id && i.status === "Cleared")
//                                       .map(i => i.interview_round)
//                                       .join(",")
//                                     router.push(
//                                       `/Event?applicantId=${encodeURIComponent(candidate.id)}` +
//                                       `&applicantName=${encodeURIComponent(candidate.applicant_name)}` +
...
//                         {selectedCandidate && (() => {
//                           const allCandidateInterviews = allInterviews
//                             .filter((int: any) => int.job_applicant === selectedCandidate.id)
//                             .sort((a: any, b: any) =>
//                               new Date(b.scheduled_on || b.creation).getTime() - new Date(a.scheduled_on || a.creation).getTime()
//                             )

//                           // Keep only the latest interview per round
//                           const seen = new Set<string>()
//                           const candidateInterviews: any[] = []
//                           for (const interview of allCandidateInterviews) {
//                             const key = interview.interview_round || interview.name
//                             if (!seen.has(key)) {
//                               seen.add(key)
...
//                           candidateInterviews.sort((a: any, b: any) =>
//                             new Date(a.scheduled_on || a.creation).getTime() - new Date(b.scheduled_on || b.creation).getTime()
//                           )
...
//                               <div className="ip-sub-head">
//                                 <div className="ip-sub-title"><Calendar size={14} /> Interview Timeline</div>
//                                 <span className="ip-badge outline">{candidateInterviews.length} Rounds</span>
//                               </div>
//                               <div className="ip-timeline-items">
//                                 {candidateInterviews.map((interview: any, index: number) => {
//                                   const isCurrent = index === candidateInterviews.length - 1
