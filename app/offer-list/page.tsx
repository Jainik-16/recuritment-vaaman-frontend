...
//     const fetchWorkflowActions = async (name: string) => {
//         try {
//             const res = await fetch(`${API_BASE_URL}.get_offer_workflow_actions?job_offer_name=${encodeURIComponent(name)}`, {
//                 credentials: 'include', headers: { 'Content-Type': 'application/json' },
//             })
//             const jsonData = await res.json()
//             setWorkflowActions(jsonData?.message?.data?.actions || [])
//         } catch (err) {
...
//     const applyWorkflowAction = async (action: string) => {
...
//                 `${API_BASE_URL}.apply_offer_workflow_action?name=${encodeURIComponent(selectedOffer.name)}&action=${encodeURIComponent(action)}`,
//                 {
//                     method: "POST",
//                     credentials: "include",
//                     headers: {
...
//                 await fetchOfferDetails(selectedOffer.name)
//                 await fetchOffers()
//                 alert(jsonData.message.message || `${action} applied successfully`)
//             } else {
//                 alert(jsonData?.message?.message || `Failed to apply "${action}"`)
//             }
//         } catch (err) {
//             console.error("Error applying workflow action:", err)
//             alert("Error applying workflow action.")
//         } finally {
...
//                                                     >
//                                                         <FileCheck size={14} /> {applyingAction ? 'Applying...' : 'Actions'}
//                                                     </button>
//                                                     {showActionsMenu && (
//                                                         <div style={{
...
//                                             <button
//                                                 className="ofl-btn"
//                                                 style={{ background: showCandidateDetails ? 'var(--accent-h)' : 'linear-gradient(135deg, #a855f7, #7e22ce)' }}
//                                                 onClick={() => setShowCandidateDetails(v => !v)}
//                                             >
//                                                 <User size={14} /> {showCandidateDetails ? 'Hide Candidate Details' : 'Candidate Details'}
//                                             </button>
//                                             <button className="ofl-detail-back" onClick={() => { setSelectedOffer(null); setEditingStatus(false); setShowCandidateDetails(false); setShowActionsMenu(false) }}>
...
//                                                                             <span className={`ofl-badge ${getStatusBadgeClass(offer.workflow_state)}`}>{offer.workflow_state || "Draft"}</span>
...
//                                                     {/* Status */}
//                                                     {/* Approval Status (workflow) — read-only, driven by Actions button */}
//                                                     <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
//                                                         <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'rgba(255,255,255,.5)' }}>Approval</span>
//                                                         <span className={`ofl-badge ${getStatusBadgeClass(selectedOffer.workflow_state)}`} style={{ fontSize: 12, padding: '4px 12px' }}>
//                                                             {selectedOffer.workflow_state || "Draft"}
//                                                         </span>
//                                                     </div>

//                                                     {/* Candidate Status — editable via pencil */}
//                                                     {editingStatus ? (
...
//                                                     ) : (
//                                                         <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
//                                                             <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'rgba(255,255,255,.5)' }}>Candidate</span>
...
//                                                 <div>
//                                                     <p className="ofl-sec-label">Candidate Details</p>
//                                                     <div className="ofl-info-grid">
//                                                         {[
//                                                             { icon: <Building2 size={16} />, cls: "indigo", label: "Current Company", val: selectedOffer.custom_current_company },
//                                                             { icon: <FileText size={16} />, cls: "amber", label: "Qualification", val: selectedOffer.custom_qualification },
//                                                             { icon: <Briefcase size={16} />, cls: "blue", label: "Total Experience", val: selectedOffer.custom_total_experience },
//                                                             { icon: <FileText size={16} />, cls: "green", label: "Our Budget (CTC)", val: selectedOffer.custom_our_budget_in_ctc_only },
//                                                             { icon: <FileText size={16} />, cls: "purple", label: "Current CTC", val: selectedOffer.custom_current_ctc_of_candidate },
//                                                             { icon: <FileText size={16} />, cls: "teal", label: "CTC to be Offered", val: selectedOffer.custom_ctc_to_be_offered },
//                                                             { icon: <UserCheck size={16} />, cls: "indigo", label: "Interviewed By Vaaman", val: selectedOffer.custom_interviewed_by_vaaman },
//                                                             { icon: <UserCheck size={16} />, cls: "purple", label: "Interviewed By Client", val: selectedOffer.custom_interviewed_by_client },
//                                                             { icon: <Briefcase size={16} />, cls: "amber", label: "Replacement/New Position", val: selectedOffer.custom_replacementnew_position },
//                                                             { icon: <Users size={16} />, cls: "teal", label: "Source of Hiring", val: selectedOffer.custom_source_of_hiring },
//                                                         ].map(item => (
//                                                             <div key={item.label} className="ofl-info-item">
//                                                                 <div className={`ofl-info-icon ${item.cls}`}>{item.icon}</div>
//                                                                 <div style={{ flex: 1, minWidth: 0 }}>
...
//                                                     {selectedOffer.custom_background_verification_done_details && (
//                                                         <div style={{ marginTop: 12 }} className="ofl-info-item">
//                                                             <div className="ofl-info-icon amber"><FileCheck size={16} /></div>
//                                                             <div style={{ flex: 1, minWidth: 0 }}>
//                                                                 <div className="ofl-info-lbl">Background Verification Details</div>
//                                                                 <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--t1)', marginTop: 2, whiteSpace: 'pre-wrap' }}>
//                                                                     {selectedOffer.custom_background_verification_done_details}
//                                                                 </div>
//                                                             </div>
//                                                         </div>
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ofl-detail-hero-left { display: flex; align-items: center; gap: 16px; flex-shrink: 0; }
  .ofl-detail-hero-left-top { display: flex; align-items: center; gap: 16px; }
  .ofl-hero-divider { width: 1px; align-self: stretch; background: rgba(255,255,255,.14); flex-shrink: 0; }
  .ofl-detail-created-by {
    display: flex; align-items: center; gap: 10px; flex-shrink: 0;
  }
  .ofl-detail-status-group { display: flex; align-items: center; gap: 32px; flex-shrink: 0; }
  .ofl-detail-status-item { display: flex; align-items: center; gap: 10px; }
  .ofl-detail-avatar {
    width: 60px; height: 60px; border-radius: 50%; flex-shrink: 0;
    background: linear-gradient(135deg, var(--accent), #3b5bdb);
    display: flex; align-items: center; justify-content: center;
  ...
  .ofl-info-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
  .ofl-info-grid-3 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
  .ofl-info-item {
    background: var(--card); border: 1px solid var(--border-s); border-radius: 10px; padding: 14px 16px;
    display: flex; align-items: center; gap: 12px;
  ...
  @media (max-width: 768px) {
    ...
    .ofl-sec-head { padding: 12px 14px; }
    .ofl-pagination { flex-direction: column; align-items: flex-start; gap: 10px; }
    .ofl-detail-hero { flex-direction: column; align-items: flex-start; gap: 16px; }
    .ofl-hero-divider { display: none; }
    .ofl-total-pill { padding: 5px 10px; }
    .ofl-total-val { font-size: 16px; }
    .ofl-total-label { font-size: 9.5px; }
    .ofl-hdr-right { width: 100%; justify-content: flex-end; margin-left: 0; flex-wrap: wrap; gap: 6px; }
                                            ...
                                            {workflowActions.length > 0 && (
                                                <div style={{ position: 'relative' }}>
                                                    <button
                                                        ...
                                                        disabled={applyingAction}
                                                    >
                                                        <Settings2 size={14} /> {applyingAction ? 'Applying...' : 'Actions'}
                                                    </button>
                                                    {showActionsMenu && (
                                                        <div style={{
                                                            position: 'absolute', top: '110%', right: 0, zIndex: 20,
                                    ...
                                    {!selectedOffer ? (
                                        ...
                                        <div className="ofl-sec-body" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                                            ...
                                            <div className="ofl-detail-hero">
                                                {/* ── Avatar / Name / ID ── */}
                                                <div className="ofl-detail-hero-left">
                                                    <div className="ofl-detail-hero-left-top">
                                                        <div className="ofl-detail-avatar">{(selectedOffer.applicant_name || "?").charAt(0).toUpperCase()}</div>
                                                        <div>
                                                            <div className="ofl-detail-name">{selectedOffer.applicant_name}</div>
                                                            <div className="ofl-detail-id">ID: {selectedOffer.name}</div>
                                                ...
                                                {selectedOffer.owner && <div className="ofl-hero-divider" />}

                                                {/* ── Created By ── */}
                                                {selectedOffer.owner && (
                                                    <div className="ofl-detail-created-by">
                                                        <div style={{
                                                        ...
                                                        <div>
                                                            <div style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.07em', color: 'rgba(255,255,255,.5)', marginBottom: 3 }}>Created By</div>
                                                            <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', lineHeight: 1.25 }}>{selectedOffer.owner.split('@')[0]}</div>
                                                        </div>
                                                    </div>
                                                )}

                                                <div className="ofl-hero-divider" />

                                                {/* ── Status + Candidate side by side ── */}
                                                <div className="ofl-detail-status-group">
                                                    {/* Approval Status (workflow) — read-only, driven by Actions button */}
                                                    <div className="ofl-detail-status-item">
                                                        <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: 'rgba(255,255,255,.5)' }}>Status</span>
                                                        <span className={`ofl-badge ${getStatusBadgeClass(selectedOffer.workflow_state)}`} style={{ fontSize: 12, padding: '4px 12px' }}>
                                                            {selectedOffer.workflow_state || "Draft"}
                                                        </span>
                                                    </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="ofl-info-grid">
                                                        {[
                                                            ...
                                                            { icon: <Users size={16} />, cls: "teal", label: "Source of Hiring", val: selectedOffer.custom_source_of_hiring },
                                                            ...(selectedOffer.custom_background_verification_done_details
                                                                ? [{ icon: <FileCheck size={16} />, cls: "amber", label: "Background Verification Details", val: selectedOffer.custom_background_verification_done_details }]
                                                                : []),
                                                        ].map(item => (
                                                            <div key={item.label} className="ofl-info-item">
                                                                <div className={`ofl-info-icon ${item.cls}`}>{item.icon}</div>
