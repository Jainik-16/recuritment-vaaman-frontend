...
//     //         setEditingComment(selectedCandidate.custom_comments || "")
//     //     }
...
export default function CandidatesPageWithJoining() {
    ...
    // ── NEW: Fetch comments from Frappe native Comment doctype ──
    const fetchComments = async (candidateId: string) => {
        setLoadingComments(true)
        try {
            const res = await fetch(
                `${API_BASE_URL}/api/resource/Comment?filters=[["reference_doctype","=","Job Applicant"],["reference_name","=","${candidateId}"],["comment_type","=","Comment"]]&fields=["name","comment_by","content","creation"]&order_by=creation asc&limit_page_length=50`,
    ...
    // ── NEW: Load comments whenever selected candidate changes ──
    useEffect(() => {
        if (selectedCandidate) {
    ...
    // ── NEW: Format comment date with time ───────────────────
                                                            ...
                                                            <div key={c.name} className="cp-comment-item">
                                                                <div className="cp-comment-header">
                                                                    ...
                                                                    <div className="cp-comment-meta">
                                                                        <span className="cp-comment-by">{c.comment_by || "Unknown"}</span>
                                                                        <span className="cp-comment-time">
                                                                            {formatCommentDate(c.creation)}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                                {/* Frappe stores content as HTML — render safely */}
                                                                <div
                                                                    className="cp-comment-text"
