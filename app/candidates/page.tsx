...
//     font-size: 13px; color: var(--t1); appearance: none;
//     outline: none; cursor: pointer; transition: all .15s;
//   }
...
//     border: 2px solid rgba(0,158,247,.25); border-top-color: var(--accent);
...
//     outline: none; resize: vertical; min-height: 72px; transition: all .15s; line-height: 1.5;
//   }
...
//     // ── Fetch all locations from Frappe ──────────────────────
//     const fetchLocations = async () => {
//         try {
//             // Try custom_location doctype first (same as job opening)
//             const res = await fetch(
//                 `${API_BASE_URL}/api/resource/custom_location?fields=["name","location_name"]&limit_page_length=0`,
...
//                 }
//             }
//             // Fallback: try "Location" doctype
//             const res2 = await fetch(
//                 `${API_BASE_URL}/api/resource/Location?fields=["name"]&limit_page_length=0`,
//                 { credentials: 'include', headers: { 'Content-Type': 'application/json' } }
//             )
//             if (res2.ok) {
//                 const data2 = await res2.json()
//                 const locs2 = (data2?.data || []).map((l: any) => l.name).filter(Boolean)
//                 setLocations(locs2)
//             }
//         } catch (e) {
...
//     // ── Update candidate location in Frappe ──────────────────
...
//     //         const res = await fetch(`${API_BASE_URL}/api/resource/Job Applicant/${candidateId}`, {
//     //             method: 'PUT',
//     //             credentials: 'include',
//     //             headers: {
...
//     //             },
//     //             body: JSON.stringify({ custom_location: newLocation }),
//     //         })
//     //         if (res.ok) {
//     //             // Optimistically update local state
//     //             setSelectedCandidate(prev =>
//     //                 prev ? { ...prev, custom_location: newLocation, location: newLocation } : prev
//     //             )
//     //             setCandidates(prev =>
//     //                 prev.map(c =>
//     //                     c.id === candidateId
//     //                         ? { ...c, custom_location: newLocation, location: newLocation }
//     //                         : c
//     //                 )
//     //             )
...
//         try {
//             const csrfToken = await getFrappeCSRF()
//             // Use set_value instead of PUT — more reliable for single field updates
//             const res = await fetch(`${API_BASE_URL}/api/method/frappe.client.set_value`, {
//                 method: 'POST',
//                 credentials: 'include',
//                 headers: {
//                     'Content-Type': 'application/json',
...
//             if (res.ok) {
//                 setSelectedCandidate(prev =>
//                     prev ? { ...prev, custom_location: newLocation } : prev
//                 )
//                 setCandidates(prev =>
//                     prev.map(c =>
//                         c.id === candidateId
//                             ? { ...c, custom_location: newLocation }
//                             : c
//                     )
//                 )
//                 setLocationSaved(true)
...
//             const csrfToken = await getFrappeCSRF()
//             const res = await fetch(
//                 `${API_BASE_URL}/api/method/frappe.client.get_list`,
//                 {
//                     method: 'POST',
//                     credentials: 'include',
//                     headers: { 'Content-Type': 'application/json', 'X-Frappe-CSRF-Token': csrfToken },
//                     body: JSON.stringify({
//                         doctype: "Comment",
//                         fields: ["name", "comment_by", "content", "comment_type", "creation"],
//                         filters: [
//                             ["Comment", "reference_doctype", "=", "Job Applicant"],
//                             ["Comment", "reference_name", "=", candidateId],
...
//             if (userIds.length > 0) {
//                 try {
//                     const userRes = await fetch(`${API_BASE_URL}/api/method/frappe.client.get_list`, {
//                         method: 'POST', credentials: 'include',
//                         headers: { 'Content-Type': 'application/json', 'X-Frappe-CSRF-Token': csrfToken },
//                         body: JSON.stringify({ doctype: "User", fields: ["name", "full_name"], filters: [["User", "name", "in", userIds]], limit_page_length: 50 })
//                     })
//                     const userData = await userRes.json()
//                     const users: any[] = userData?.message || []
//                     users.forEach((u: any) => { userMap[u.name] = u.full_name || u.name })
//                 } catch (e) { console.error("Error fetching user names:", e) }
//             }
//             const enrichedComments = list.map((c: any) => ({
//                 ...c,
//                 display_name: c.comment_by ? (userMap[c.comment_by] || c.comment_by) : "Unknown User"
//             }))
...
//             const res = await fetch(`${API_BASE_URL}/api/resource/Comment`, {
//                 method: 'POST', credentials: 'include',
//                 headers: { 'Content-Type': 'application/json', 'X-Frappe-CSRF-Token': csrfToken },
//                 body: JSON.stringify({
//                     comment_type: "Comment",
//                     reference_doctype: "Job Applicant",
//                     reference_name: selectedCandidate.id,
//                     content: newComment.trim(),
//                     comment_by: loggedInUser,
//                 }),
//             })
//             if (res.ok) {
//                 setNewComment("")
...
//     useEffect(() => {
//         fetchCandidates()
//         fetchLocations()   // ← fetch locations alongside candidates
//     }, [])
...
//     const getCommentInitials = (c: CandidateComment) => {
...
//     // ── Current location value for dropdown ──────────────────
//     const getCurrentLocationValue = (candidate: Candidate) =>
//         candidate.custom_location || ""
...
//                                 <Link key={s.href} href={s.href} className={`cp-nav-link${s.href === "/candidates" ? " active" : ""}`}>
//                                     {s.icon} {s.title}
//                                 </Link>
//                             ))}
//                             <div className="cp-nav-lbl" style={{ marginTop: 12 }}>Closing</div>
//                             {sidebarClosing.map(s => (
//                                 <Link key={s.href} href={s.href} className="cp-nav-link">{s.icon} {s.title}</Link>
//                             ))}
//                         </nav>
...
//                                 <Home size={13} /> Home <ChevronRight size={13} /> <strong>Candidates</strong>
//                             </div>
//                         </header>
...
//                                             <Search size={15} />
//                                             <input type="text" className="cp-search" placeholder="Search by name, email, phone, or job title..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
//                                         </div>
...
//                                                         ? "Try adjusting your filters" : "No candidates available"}
//                                                 </p>
//                                             </div>
//                                         ) : (
...
//                                                                     <div className="cp-card-meta"><Briefcase size={11} /> {candidate.designation}</div>
//                                                                 )}
//                                                                 {candidate.job_title && (
...
//                                                                     </div>
//                                                                     <div className="cp-card-contact"><Phone size={11} /> {candidate.phone_number}</div>
//                                                                 </div>
//                                                             </div>
//                                                             <div className="cp-card-right">
//                                                                 <span className="cp-badge blue">{candidate.status}</span>
//                                                                 {candidate.resume_score > 0 && <span className="cp-score">{candidate.resume_score}%</span>}
//                                                             </div>
//                                                         </div>
...
//                                                             value={getCurrentLocationValue(selectedCandidate)}
//                                                             onChange={e => updateCandidateLocation(selectedCandidate.id, e.target.value)}
//                                                             disabled={updatingLocation}
//                                                         >
//                                                             <option value="">— Select Location —</option>
//                                                             {/* Always include current value even if not in fetched list */}
//                                                             {getCurrentLocationValue(selectedCandidate) &&
//                                                                 !locations.includes(getCurrentLocationValue(selectedCandidate)) && (
//                                                                     <option value={getCurrentLocationValue(selectedCandidate)}>
//                                                                         {getCurrentLocationValue(selectedCandidate)}
//                                                                     </option>
//                                                                 )
//                                                             }
...
//                                                             <div className="cp-loc-spin" /> Saving location...
//                                                         </div>
//                                                     )}
//                                                     {locationSaved && !updatingLocation && (
//                                                         <div className="cp-loc-saved">
//                                                             <CheckCircle size={11} /> Location saved
//                                                         </div>
//                                                     )}
//                                                 </div>
...
//                                                 <div>
//                                                     <div className="cp-detail-label" style={{ marginBottom: 10 }}>
//                                                         <MessageSquare size={11} />
//                                                         Comments {comments.length > 0 ? `(${comments.length})` : ""}
//                                                     </div>
//                                                     <div className="cp-comments-section">
//                                                         {loadingComments && (
//                                                             <div className="cp-comment-loading">
//                                                                 <div className="cp-comment-spin" /> Loading comments...
//                                                             </div>
//                                                         )}
//                                                         {!loadingComments && comments.length === 0 && (
//                                                             <p className="cp-comment-empty">No comments yet. Be the first to comment.</p>
//                                                         )}
...
//                                                                         <span className="cp-comment-by">{getCommentAuthor(c)}</span>
//                                                                         <span className="cp-comment-time">{formatCommentDate(c.creation)}</span>
//                                                                     </div>
//                                                                 </div>
...
//                                                             />
//                                                             <button className="cp-comment-save" onClick={saveComment} disabled={savingComment || !newComment.trim()}>
//                                                                 {savingComment ? "Saving..." : <><Send size={13} /> Add Comment</>}
//                                                             </button>
//                                                         </div>
//                                                     </div>
//                                                 </div>
...
//                                                     return (
//                                                         <>
//                                                             <button className="cp-action-btn" disabled={isInterviewCompleted} onClick={() => !isInterviewCompleted && router.push(`/interview?applicantId=${selectedCandidate.id}`)}>
//                                                                 <Users size={14} /> Schedule Interview
//                                                                 {isInterviewCompleted && <span className="cp-action-completed">(Completed)</span>}
//                                                             </button>
//                                                             <button className="cp-action-btn" disabled={isFeedbackCompleted} onClick={() => !isFeedbackCompleted && router.push(`/candidate-feedback?candidateId=${selectedCandidate.id}`)}>
//                                                                 <ClipboardList size={14} /> Add Feedback
//                                                                 {isFeedbackCompleted && <span className="cp-action-completed">(Completed)</span>}
//                                                             </button>
//                                                             <button className="cp-action-btn" disabled={isDocVerifyCompleted} onClick={() => !isDocVerifyCompleted && router.push(`/document-verify?candidateId=${selectedCandidate.id}`)}>
//                                                                 <FileCheck size={14} /> Verify Documents
//                                                                 {isDocVerifyCompleted && <span className="cp-action-completed">(Completed)</span>}
//                                                             </button>
//                                                             <button className="cp-action-btn" disabled={isOfferLetterCompleted} onClick={() => !isOfferLetterCompleted && router.push(`/offer-letter?candidateId=${selectedCandidate.id}`)}>
//                                                                 <Send size={14} /> Send Offer Letter
//                                                                 {isOfferLetterCompleted && <span className="cp-action-completed">(Completed)</span>}
//                                                             </button>
//                                                             <button className="cp-action-btn" style={{ marginBottom: 4 }} disabled={isAppointmentCompleted} onClick={() => !isAppointmentCompleted && router.push(`/letter-appointment?candidateId=${selectedCandidate.id}`)}>
...
//     // ── Read jobOpening filter from URL ──────────────────────
...
//                     .map((l: any) => l.location_name || l.name)
//                     .filter(Boolean)
//                 if (locs.length > 0) { setLocations(locs); return }
//             }
//             const res2 = await fetch(
//                 `${API_BASE_URL}/api/resource/Location?fields=["name"]&limit_page_length=0`,
//                 { credentials: 'include', headers: { 'Content-Type': 'application/json' } }
...
//             if (res2.ok) {
//                 const data2 = await res2.json()
//                 setLocations((data2?.data || []).map((l: any) => l.name).filter(Boolean))
//             }
...
//                 method: 'POST', credentials: 'include',
//                 headers: { 'Content-Type': 'application/json', 'X-Frappe-CSRF-Token': csrfToken },
//                 body: JSON.stringify({ doctype: 'Job Applicant', name: candidateId, fieldname: 'custom_location', value: newLocation }),
//             })
//             if (res.ok) {
...
//     // REPLACE entire fetchCandidates with this:
//     const fetchCandidates = async () => {
//         setIsLoading(true)
//         setApiError(null)
//         try {
...
//             const data = await response.json()
//             if (data && data.data) {
//                 // ── STEP 1: Map candidates immediately WITHOUT stage statuses ──
...
//                     job_title: item.job_title || "Not specified",
...
//                     resume_score: item.resume_score || 0,
//                     custom_recruitment_stage: item.custom_recruitment_stage || "",
//                     stage_statuses: [], // empty initially
//                     justification_by_ai: item.justification_by_ai || "",
//                     applicant_rating: item.applicant_rating || 0,
...
//                 // ── STEP 3: Fetch stage statuses in background in small batches ──
//                 const BATCH_SIZE = 5
//                 for (let i = 0; i < mappedData.length; i += BATCH_SIZE) {
//                     const batch = mappedData.slice(i, i + BATCH_SIZE)
//                     const batchResults = await Promise.all(
//                         batch.map(async (candidate: Candidate) => {
//                             const statuses = await fetchCandidateStageStatuses(candidate.id)
//                             return { id: candidate.id, statuses }
//                         })
//                     )
//                     // Update only the candidates in this batch
//                     setCandidates(prev => prev.map(c => {
//                         const result = batchResults.find(r => r.id === c.id)
//                         return result ? { ...c, stage_statuses: result.statuses } : c
//                     }))
...
//                 alert("Joining confirmation updated successfully!")
//             } else {
//                 alert(`Error: ${result.message || "Failed to update joining confirmation"}`)
//                 await fetchCandidates()
//             }
//         } catch (error: any) {
//             alert(`Error: ${error.message || "Failed to update joining confirmation"}`)
...
//                 method: 'POST', credentials: 'include',
//                 headers: { 'Content-Type': 'application/json', 'X-Frappe-CSRF-Token': csrfToken },
//                 body: JSON.stringify({ comment_type: "Comment", reference_doctype: "Job Applicant", reference_name: selectedCandidate.id, content: newComment.trim(), comment_by: loggedInUser }),
//             })
...
//     useEffect(() => { fetchCandidates(); fetchLocations() }, [])
//     useEffect(() => { document.title = 'Candidates' }, [])

//     useEffect(() => {
//         if (selectedCandidate) { fetchComments(selectedCandidate.id); setNewComment(""); setLocationSaved(false) }
//         else { setComments([]) }
//     }, [selectedCandidate?.id])

//     // ── FILTERING (includes jobOpening from URL param) ────────
//     useEffect(() => {
//         let filtered = candidates

//         // Apply job opening filter from URL first
//         if (jobOpeningFilter) {
//             filtered = filtered.filter(c => c.job_opening === jobOpeningFilter)
//         }
...
//     // Stats computed from ALL candidates (not filtered by job opening)
//     const allCandidates = jobOpeningFilter
//         ? candidates.filter(c => c.job_opening === jobOpeningFilter)
//         : candidates
...
//                             <Link href="/home" className="cp-nav-link">
//                                 <Home size={15} /> Home
//                             </Link>
//                             <div className="cp-nav-lbl">Pipeline</div>
//                             {sidebarPipeline.map(s => (
...
//                                 {jobOpeningFilter && (
//                                     <>
//                                         <Link href="/job-opening" style={{ color: 'var(--t3)', textDecoration: 'none' }}>Job Openings</Link>
...
//                                         <div className="cp-panel-title-row">
//                                             <Users size={15} style={{ color: 'var(--accent)' }} />
//                                             <span className="cp-panel-title">
//                                                 Candidates ({filteredCandidates.length})
//                                                 {jobOpeningFilter && <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--accent)', marginLeft: 8 }}>— {jobOpeningFilter}</span>}
//                                             </span>
//                                         </div>
//                                         <div className="cp-search-wrap">
//                                             <Search size={15} />
...
//                                                 <p className="cp-empty-sub">
//                                                     {jobOpeningFilter
//                                                         ? `No applicants found for ${jobOpeningFilter}`
//                                                         : searchTerm || filterStatus !== "all" || filterDesignation !== "all"
//                                                             ? "Try adjusting your filters" : "No candidates available"}
//                                                 </p>
//                                             </div>
...
//                                                     </div>
//                                                     {updatingLocation && (
//                                                         <div className="cp-loc-saving"><div className="cp-loc-spin" /> Saving location...</div>
//                                                     )}
//                                                     {locationSaved && !updatingLocation && (
//                                                         <div className="cp-loc-saved"><CheckCircle size={11} /> Location saved</div>
//                                                     )}
//                                                 </div>
...
//                                                     <div className="cp-comments-section">
//                                                         {loadingComments && (
//                                                             <div className="cp-comment-loading"><div className="cp-comment-spin" /> Loading comments...</div>
//                                                         )}
//                                                         {!loadingComments && comments.length === 0 && (
//                                                             <p className="cp-comment-empty">No comments yet. Be the first to comment.</p>
//                                                         )}
